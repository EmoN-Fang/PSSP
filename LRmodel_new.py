...
def initialize_parameters():
    ...
    W1 = tf.get_variable(
        "W1", [100, 300], initializer=tf.contrib.layers.xavier_initializer(seed=3))
    b1 = tf.get_variable("b1", [100, 1], initializer=tf.zeros_initializer())
    W2 = tf.get_variable(
        "W2", [20, 100], initializer=tf.contrib.layers.xavier_initializer(seed=3))
    b2 = tf.get_variable("b2", [20, 1], initializer=tf.zeros_initializer())
    W3 = tf.get_variable(
        "W3", [3, 20], initializer=tf.contrib.layers.xavier_initializer(seed=3))
...
#     learning_rate = tf.train.exponential_decay(learning_rate0, current_iter, 3500, 0.9, staircase=False)
    optimizer = tf.train.AdamOptimizer(
        learning_rate=learning_rate0).minimize(cost)
